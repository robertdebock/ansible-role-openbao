...
# Whether to persist the environment file to disk after OpenBao reads it.
# When false (default), the file /etc/openbao/openbao.env is deleted after
# OpenBao starts and initialization completes, removing sensitive values from disk.
# When true, the file persists (for backward compatibility).
# Set to true if you need the environment variables to persist across service restarts.
openbao_service_environment_persist: false
